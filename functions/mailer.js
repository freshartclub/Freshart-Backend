	...
	return new Promise(async function (resolve, reject) {
		try {
			...
			for (let key in mailVariable) {
				if (subject) subject = subject.replaceAll(key, mailVariable[key]);
				if (html) html = html.replaceAll(key, mailVariable[key]);
				if (text) text = text.replaceAll(key, mailVariable[key]);
			}
			// Prepare the options
			const options = {
