      ...
      return resolve({
        ...
        message:
          JSON.parse(data) === true
            ? "Art enabled succcessfully"
            : "Art disabled succcessfully",
      });
    } catch (error) {
      // Reject the process
      return reject(error);
...
module.exports.resetPasswordforArtistorUser = (token, password) => {
  return new Promise(async function (resolve, reject) {
    try {
      ...
      if (userData) {
        User.updateOne(
          { token: token, isDeleted: false, status: "active" },
          {
            $unset: { token: "" },
            $set: { password: md5(password.newPassword), tokens: [] },
          }
        ).then();
      } else {
        const artistData = await Artist.findOne({
...
module.exports.commericalUsersFunc = (commArray, users) => {
  return new Promise(async function (resolve, reject) {
    try {
      const userIdData = (
        await User.find({}, { userId: 1 }).sort({ _id: -1 })
      )[0];
